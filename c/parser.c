...
static GumboInsertionMode get_appropriate_insertion_mode(const GumboParser * parser, int index)
{
    ...
    case GUMBO_TAG_HTML:
        return parser->_parser_state->_head_element ? GUMBO_INSERTION_MODE_AFTER_HEAD : GUMBO_INSERTION_MODE_BEFORE_HEAD;
    default:
        break;
    }
    return is_last ? GUMBO_INSERTION_MODE_IN_BODY : GUMBO_INSERTION_MODE_INITIAL;
    ...
    GumboParser * parser, GumboNode * override_target)
{
    ...
    if (retval.target == NULL) {
        ...
        // non-empty.
        retval.target = parser->_output->root != NULL ? get_current_node(parser) : get_document_node(parser);
    }
...
static void record_end_of_element(GumboToken * current_token, GumboElement * element)
{
    ...
    element->original_end_tag = current_token->type == GUMBO_TOKEN_END_TAG ? current_token->original_text : kGumboEmptyString;
}
...
static GumboNode* create_element(GumboParser * parser, GumboTag tag)
{
    ...
    element->start_pos = (parser->_parser_state->_current_token) ? parser->_parser_state->_current_token->position : kGumboEmptySourcePosition;
...
static GumboNode* create_element_from_token(GumboParser * parser, GumboToken * token, GumboNamespaceEnum tag_namespace)
{
    ...
    GumboNodeType type = (tag_namespace == GUMBO_NAMESPACE_HTML && start_tag->tag == GUMBO_TAG_TEMPLATE) ? GUMBO_NODE_TEMPLATE : GUMBO_NODE_ELEMENT;
...
static void insert_element(GumboParser * parser, GumboNode * node, bool is_reconstructing_formatting_elements)
{
    GumboParserState * state = parser->_parser_state;
...
static GumboNode* insert_foreign_element(GumboParser * parser, GumboToken * token, GumboNamespaceEnum tag_namespace)
{
    ...
    if (token_has_attribute(token, "xmlns") &&
        !attribute_matches_case_sensitive(&token->v.start_tag.attributes, "xmlns", kLegalXmlns[tag_namespace])) {
    ...
    if (token_has_attribute(token, "xmlns:xlink") &&
        !attribute_matches_case_sensitive(&token->v.start_tag.attributes, "xmlns:xlink", "http://www.w3.org/1999/xlink")) {
        parser_add_parse_error(parser, token);
    }
    return element;
}
...
static int count_formatting_elements_of_tag(GumboParser * parser, const GumboNode * desired_node, int * earliest_matching_index)
{
...
static void reconstruct_active_formatting_elements(GumboParser * parser)
{
    ...
    ++i;
    gumbo_debug("Reconstructing elements from %d on %s parent.\n", i, gumbo_normalized_tagname(get_current_node(parser)->v.element.tag));
...
static bool has_an_element_in_specific_scope(GumboParser * parser, int expected_size, const GumboTag * expected, bool negate, const gumbo_tagset tags)
{
    GumboVector * open_elements = &parser->_parser_state->_open_elements;
    for (int i = open_elements->length; --i >= 0;) {
...
static bool implicitly_close_tags(GumboParser * parser, GumboToken * token, GumboNamespaceEnum target_ns, GumboTag target)
{
    bool result = true;
...
static bool doctype_matches(const GumboTokenDocType * doctype, const GumboStringPiece * public_id, const GumboStringPiece * system_id, bool allow_missing_system_id)
{
