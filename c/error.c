...
static int print_message(GumboParser * parser, GumboStringBuffer * output, const char * format, ...)
{
    ...
    if (bytes_written == -1) {
        ...
        va_start(args, format);
        int result = vsnprintf(output->data + output->length, remaining_capacity, format, args);
        va_end(args);
...
void gumbo_error_to_string(GumboParser * parser, const GumboError * error, GumboStringBuffer * output)
{
    ...
    case GUMBO_ERR_NUMERIC_CHAR_REF_WITHOUT_SEMICOLON:
        print_message(parser, output, "The numeric character reference &#%d should be followed by a semicolon", error->v.codepoint);
        break;
    case GUMBO_ERR_NUMERIC_CHAR_REF_INVALID:
        print_message(parser, output, "The numeric character reference &#%d; encodes an invalid unicode codepoint", error->v.codepoint);
        break;
    case GUMBO_ERR_NAMED_CHAR_REF_WITHOUT_SEMICOLON:
        print_message(parser, output, "The named character reference &%.*s should be followed by a semicolon", (int)error->v.text.length, error->v.text.data);
        break;
...
{
    if (error->type == GUMBO_ERR_PARSER || error->type == GUMBO_ERR_UNACKNOWLEDGED_SELF_CLOSING_TAG) {
