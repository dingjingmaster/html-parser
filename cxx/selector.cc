...
bool Selector::match(GumboNode * apNode)
{
    switch (mOp) {
    case EDummy:
        return true;
...
std::vector<GumboNode*> Selector::filter(std::vector<GumboNode*> nodes)
{
...
std::vector<GumboNode*> Selector::matchAll(GumboNode * apNode)
{
    std::vector<GumboNode*> ret;
...
void Selector::matchAllInto(GumboNode * apNode, std::vector<GumboNode*>& nodes)
{
    if (match(apNode)) {
...
bool BinarySelector::match(GumboNode * apNode)
{
    switch (mOp) {
    case EUnion:
...
bool AttributeSelector::match(GumboNode * apNode)
{
    if (apNode->type != GUMBO_NODE_ELEMENT) {
        return false;
    }
...
bool UnarySelector::hasDescendantMatch(GumboNode * apNode, Selector * apS)
{
    for (unsigned int i = 0; i < apNode->v.element.children.length; i++) {
        GumboNode * child = (GumboNode*)apNode->v.element.children.data[i];
        if (apS->match(child)
...
bool UnarySelector::hasChildMatch(GumboNode * apNode, Selector * apS)
{
    for (unsigned int i = 0; i < apNode->v.element.children.length; i++) {
        GumboNode * child = (GumboNode*)apNode->v.element.children.data[i];
        if (apS->match(child)) {
...
bool UnarySelector::match(GumboNode * apNode)
{
    switch (mOp) {
    case ENot:
...
bool TextSelector::match(GumboNode * apNode)
{
    std::string text;
    switch (mOp) {
    case EContains:
        text = QueryUtil::nodeText(apNode);
        break;
    case EOwnContains:
        text = QueryUtil::nodeOwnText(apNode);
        break;
    default:
        return false;
    }

    text = QueryUtil::tolower(text);
